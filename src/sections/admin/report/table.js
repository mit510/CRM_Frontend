...
import { ReportCloseCallTableData } from 'src/assets/data/TableData';
...
const CloseCallTable = () => {
  ...
  return (
    <TableContainer sx={{marginTop:'30px'}}>
            <Typography variant="h5" align="center" sx={{ marginBottom: '20px' }}>Report of Close Calls</Typography>
    
      <Table>
        <TableHead>
          <StyledTableRow>
            {ReportCloseCallTableData .columns.map((column, index) => (
        ...
        <TableBody>
          {ReportCloseCallTableData .rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, rowIndex) => (
      ...
      <TablePagination
        ...
        component="div"
        count={ReportCloseCallTableData .rows.length}
