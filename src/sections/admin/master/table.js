...
import { AdminTerritoryMaster } from 'src/assets/data/TableData';
...
const TerritoryTable = () => {
  ...
  return (
    <TableContainer sx={{marginTop:'30px'}}>
            <Typography variant="h5" align="center" sx={{ marginBottom: '20px' }}>Territory Information</Typography>
    
      <Table>
        <TableHead>
          <StyledTableRow>
            {AdminTerritoryMaster .columns.map((column, index) => (
        ...
        <TableBody>
          {AdminTerritoryMaster .rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, rowIndex) => (
      ...
      <TablePagination
        ...
        component="div"
        count={AdminTerritoryMaster .rows.length}
