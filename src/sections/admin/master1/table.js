...
import { AdminEngineeringMaster } from 'src/assets/data/TableData';
...
const EngineeringTable = () => {
  ...
  return (
    <TableContainer sx={{marginTop:'30px'}}>
            <Typography variant="h5" align="center" sx={{ marginBottom: '20px' }}>Engineering Information</Typography>
    
      <Table>
        <TableHead>
          <StyledTableRow>
            {AdminEngineeringMaster .columns.map((column, index) => (
        ...
        <TableBody>
          {AdminEngineeringMaster .rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, rowIndex) => (
      ...
      <TablePagination
        ...
        component="div"
        count={AdminEngineeringMaster .rows.length}
