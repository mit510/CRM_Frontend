...
import { ReportPendingCallTableData } from 'src/assets/data/TableData';
...
const PendingCall = () => {
  ...
  return (
    <TableContainer sx={{marginTop:'30px'}}>
            <Typography variant="h5" align="center" sx={{ marginBottom: '20px' }}>Report of Pending Calls</Typography>
    
      <Table>
        <TableHead>
          <StyledTableRow>
            {ReportPendingCallTableData .columns.map((column, index) => (
        ...
        <TableBody>
          {ReportPendingCallTableData .rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, rowIndex) => (
      ...
      <TablePagination
        ...
        component="div"
        count={ReportPendingCallTableData .rows.length}
