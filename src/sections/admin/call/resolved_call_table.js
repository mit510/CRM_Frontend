...
import { AdminCallRequestAcc3 } from 'src/assets/data/TableData';
...
const ResolvedCalls = () => {
  ...
  return (
    <TableContainer sx={{ marginBottom: '30px',marginTop:'30px' }}>
      <Typography variant="h5" align="center" sx={{ marginBottom: '30px'}}>Resolved Calls </Typography>
      <Table>
        <TableHead>
          <StyledTableRow>
            {AdminCallRequestAcc3.columns.map((column, index) => (
        ...
        <TableBody>
          {AdminCallRequestAcc3.rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, rowIndex) => (
      ...
      <TablePagination
        ...
        component="div"
        count={AdminCallRequestAcc3.rows.length}
