...
import { AdminCallRequestAcc1 } from 'src/assets/data/TableData';
...
const AllCalls = () => {
  ...
  return (
    ...
    <Typography variant="h5" align="center" sx={{ marginBottom: '30px'}}>Pending / Work In Progress Call</Typography>
      <Table>
        <TableHead>
          <StyledTableRow>
            {AdminCallRequestAcc1.columns.map((column, index) => (
        ...
        <TableBody>
          {AdminCallRequestAcc1.rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, rowIndex) => (
      ...
      <TablePagination
        ...
        component="div"
        count={AdminCallRequestAcc1.rows.length}
