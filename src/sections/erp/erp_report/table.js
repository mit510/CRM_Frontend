...
import { ERPCallStatusData } from 'src/assets/data/TableData';
...
const CallStatusTable = () => {
  ...
  return (
    <TableContainer sx={{ marginBottom: '30px',marginTop:'30px' }}>
      <Typography variant="h5" align="center" sx={{ marginBottom: '30px'}}>Call Status Data </Typography>
      <Table>
        <TableHead>
          <StyledTableRow>
            {ERPCallStatusData.columns.map((column, index) => (
              <StyledTableCell key={index} sx={{textAlign:'center'}}>{column.title}</StyledTableCell>
            ))}
          </StyledTableRow>
        </TableHead>
        <TableBody>
          {ERPCallStatusData.rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, rowIndex) => (
      ...
      <TablePagination
        ...
        component="div"
        count={ERPCallStatusData.rows.length}
